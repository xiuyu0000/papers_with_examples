...
from utils.tools import save_losses, save_image, get_lr
from utils.device_adapter import get_device_num, get_rank_id, get_device_id
from config.pix2pix_config import pix2pix_config as config
...
def train(arg):
    ...
    dataset = Pix2PixDataset(root_dir=arg.train_data_dir, config=config)
    ...
    if arg.device_target == 'Ascend':
        if arg.run_distribute:
            ...
            context.set_context(device_id=int(os.getenv("DEVICE_ID")), device_target="Ascend")
            ...
            context.set_context(device_id=get_device_id(), device_target="Ascend")
        ...
        if arg.run_distribute:
            ...
            context.set_context(device_id=get_device_id(), device_target="GPU")
    ...
    d_loss_fn = LossD(config)
    g_loss_fn = LossG(config)
    d_loss_net = WithLossCellD(backbone=pix2pix, loss_fn=d_loss_fn)
    g_loss_net = WithLossCellG(backbone=pix2pix, loss_fn=g_loss_fn)

    d_opt = nn.Adam(pix2pix.net_discriminator.trainable_params(), learning_rate=get_lr(config),
                    beta1=arg.beta1, beta2=arg.beta2, loss_scale=1)
    g_opt = nn.Adam(pix2pix.net_generator.trainable_params(), learning_rate=get_lr(config),
                    beta1=arg.beta1, beta2=arg.beta2, loss_scale=1)
    ...
    for epoch in range(arg.epoch_num):
        for i, data in enumerate(data_loader):
            ...
            if i == steps_per_epoch - 1:
                ...
                print("The learning rate at this point is：", get_lr(config)[epoch * i])
        ...
        # Save losses
        save_losses(g_losses, d_losses, epoch + 1, config)
        print("epoch", epoch + 1, "D&G_Losses saved")
        print("epoch", epoch + 1, "finished")
        # Save checkpoint
        if (epoch + 1) == arg.epoch_num:    #different epoch save different ckpt
            if arg.run_distribute:
